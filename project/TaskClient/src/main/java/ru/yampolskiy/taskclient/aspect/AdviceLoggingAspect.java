...
public class AdviceLoggingAspect {
    ...
    private Logger logger = LoggerFactory.getLogger(AdviceLoggingAspect.class);

    /**
     * Логирует вызов метода перед его выполнением.
     * @param joinPoint Точка присоединения, представляющая вызов метода.
     */
    @Before("execution(* ru.yampolskiy.taskclient.advice.*.*(..))")
     ...
     */
    @AfterReturning(pointcut = "execution(* ru.yampolskiy.taskclient.advice.*.*(..))", returning = "result")
     ...
     */
    @AfterThrowing(pointcut = "execution(* ru.yampolskiy.taskclient.advice.*.*(..))", throwing = "exception")
